...
import { requests, API_KEY } from "../src/requests";
...
const Search = () => {
  ...
  const fetchData = useCallback(async () => {
    return await requests(context.searchInput, null).then((response) => {
  ...
  const queryNextBatch = async (text, page) => {
    ...
    return await requests(text, nextPage).then((response) => {
      if (movies) {
  ...
  const renderPosters = (data) => {
    return data.map((item, index) => {
      if (item.poster_path)
        return (
          <div key={index}>
            <img src={`${API_KEY}w500/${item.poster_path}`} alt={"poster"} />
          </div>
        );
      return null;
    });
          ...
          {movies.length ? (
            <React.Fragment>
              ...
              {page < totalPages ? (
                <div
                  className="load-more"
                  onClick={() => loadMoreMovies(context.searchInput)}
                >
                  <span>
